...
class SIM:
    ...
    def run_initial_simulations(self, dictParams = None):
        ...
        if self.info["method"] == "auto":
            ...
            np.save(f"{self.resultPath}/initial_params.npy", n_params)
    ...
    def submit_initial_jobs(self):
        ...
        if hyperqueue == "no":
            ...
            with open("linux_slurm/hyperqueue_file.txt", 'w') as filename:
                for loading in loadings:
                    for index in range(1, self.fileIndex + 1):
                        filename.write(f"{projectPath}/{self.simulationPath}/{loading}/{index} {material}\n")
            print("Initial simulation preprocessing stage starts")
            numberOfJobsRequired = self.fileIndex * 7 
            print("Number of jobs required:", numberOfJobsRequired)
            subprocess.run(f"sbatch --wait linux_slurm/{server}_hyperqueue_pre.sh", shell=True)
        ...
        while True:
            ...
            np.save(f'{self.resultPath}/initial_params.npy', list(self.path2params["linear_uniaxial_RD"].values()))
        ...
        if hyperqueue == "no":
            ...
            numberOfJobsRequired = self.fileIndex * 7
            print("Number of jobs required:", numberOfJobsRequired) 
            subprocess.run(f"sbatch --wait linux_slurm/{server}_hyperqueue_post.sh", shell=True)
            print("Initial simulation postprocessing stage finished")
    ...
    def save_initial_outputs(self):
        ...
        np.save(f"{self.resultPath}/initial_processCurves.npy", self.initial_processCurves)
        np.save(f"{self.resultPath}/initial_trueCurves.npy", self.initial_trueCurves) 
    ...
    def submit_array_analysis_jobs(self, numberOfAnalysisCurves, numberOfJobsRequired, paramNames, middle_fileindex):
        ...
        if hyperqueue == "no":
            with open("linux_slurm/array_analysis_file.txt", 'w') as filename:
                for loading in loadings:
            ...
            subprocess.run(f"sbatch --wait --array=1-{numberOfJobsRequired} linux_slurm/{server}_array_pre.sh {material} analysis", shell=True)
            print("Parameter analysis preprocessing stage finished. The postprocessing stage starts")
            subprocess.run(f"sbatch --wait --array=1-{numberOfJobsRequired} linux_slurm/{server}_array_post.sh {material} analysis", shell=True)
    ...
    def get_grid(self, numberOfSims):
        ...
        if searchingSpace == "discrete":  
            for _ in range(numberOfSims):
                while True:    
                    candidateParam = {}
                    for param in self.info['param_info']:
                        if param_info[param]["optimized_target"] == True:
                            candidateParam[param] = self.discrete_param(self.info['param_info'][param]['low'], 
                                                                            self.info['param_info'][param]['high'], 
                                                                            self.info['param_info'][param]['step'], 
                                                                            self.info['param_info'][param]['round'])
                    if candidateParam not in points:
                        break
                points.append(candidateParam)
        return points
