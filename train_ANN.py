...
def main_trainANN(info, prepared_data):
    ...
    print(param_info)
    spacing = 100
    featureMatrixScaling = np.zeros((len(list(param_info.keys())), spacing))
    print(featureMatrixScaling)
    
    for index, parameter in enumerate(list(param_info.keys())):
        ...
        featureMatrixScaling[:, index] = np.linspace(param_info[parameter]["low"], param_info[parameter]["high"], spacing)
    print(featureMatrixScaling)
    time.sleep(180)
    start = time.time()
    for loading in loadings:
...
if __name__ == '__main__':
    ...
    prepare_data = prepare_data.main_prepareData(info)
    main_trainANN(info, prepare_data)