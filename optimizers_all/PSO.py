...
class PSO(optimizer):
    ...
    def __init__(self, info):
        ...
        #     c2: social parameter
        #     w: inertia parameter
        self.options = {'c1': 2.05, 'c2': 2.05, 'w': 0.72984}
        self.n_particles = 50
        self.iterations = 20
        self.velocity_clamp = (0.01, 100)
        self.verbose = False
            ...
            for solution_index in range(0, num_particles):
                ...
                for loading in loadings:
                    ...
                    if self.optimize_type == "yielding":
                        fitness += weightsLoading[loading] * fitnessYieldingOneLoading(exp_curves["interpolate"][loading]["stress"], predicted_sim_stress, exp_curves["interpolate"][loading]["strain"], weightsYielding)
                    elif self.optimize_type == "hardening":
                        fitness += weightsLoading[loading] * fitnessHardeningOneLoading(exp_curves["interpolate"][loading]["stress"], predicted_sim_stress,  exp_curves["interpolate"][loading]["strain"], weightsHardening)
